...
/*:
 ...
 - I'm using the HeadFirst "SimUDuck" example to illustrate the Strategy Pattern. This app models a bunch of differnt kinds of ducks that swim and quack.
 
 ![](sim.png)
 
 - We want to model duck subtypes that initially just have the `quack()`, and `swim()` behaviours. (We will add `fly()` later.)
 - The `Duck` super class gives us default implementations of `quack()` and `swim()`.
 - This avoids code duplication if the subtype can make use of the super's implementation.
 */
...
class DecoyDuck: Duck {
  ...
  override func quack() {
    // overrides and does nothing. violates Interface Segregation Principle (ISP).
  }
...
/*:
 ...
 > _Interface Segregation Principle_: For a class to depend on methods it doesn't use, violates the [Interface Segregation Principle](https://en.wikipedia.org/wiki/Interface_segregation_principle)). If our class depends on a method it doesn't even use then this means changes to the super class's method could be breaking for our subclass even though it doesn't actually use the behavior!
 ...
 - Another problem is that the more we override behavior the less we can gain knowledge of all ducks by looking at the super class.
 - Also, we can't really change behaviour at run time. Our design is static.
 - If we add another behaviour `fly()` we have to open and edit every class that overrides this behaviour. In a complex app this might be daunting and introduce new bugs. This violates the Open/Closed principle.
...
/*:
 ...
 - The simplest attempt at doing this is to move the behaviour that changes out and into a protocol/interface.
 - This way each class can conform to the protocol and implement its own fly, swim or quack behaviour.
 - If a particular Duck sub type lacks a behaviour, say it can't fly, them it simply won't conform to the Flyable protocol. Alternatively we could make a particular protocol method optional.
...
// All Concrete Ducsk implement description so we add it to a superclass, we could also just make each Ducks subtype conform
...
// This class is almost identifical to Mallard! This points to a serious problem with this solution. Why?
...
/*:
 - This solution solves our problem by making our subclasses composed of behaviors. So we are using composition rather than inheritance. If a Duck sub type doesn't have a behavior it doesn't conform to the protocol.
 - But this solution violates a fundamental principle: `DRY (Do Not Repeat Yourself)`.
 - If you had to make changes you would have to make them everywhere the implementation was the same.
 - This could be a maintenance nightmare especially in a big application.
...
/*:
 ...
 - This is similar to the protocol way except that we create classes that provide implementations of behaviours rather than locking the implementation of the protocols inside the Duck subclasses.
...
class Duck3: CustomStringConvertible {
  ...
  // It doesn't make sense to make the description method a strategy unless some duck subtypes implement the same description
  var description: String {
    fatalError()
  }
}
...
mallard.airBehavior = FlyinHigh()
mallard.waterBehavior = Swimmer()
mallard.soundBehavior = Quacker()
...
//: **We can add a new rocket power to fly without violating the Open/Closed principle**, that is, we don't need to touch any of the Duck sub types to make the change.
...
/*:
 ...
 - We can dynamically switch behaviours at run time!
 - We don't have to touch the Duck class or any subclass to add a new behavior. So, we obey the `Open/Closed Principle`.
